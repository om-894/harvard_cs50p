...
def convert(s):
    ...
    time = re.search(r"(\d{1,2}):?(\d{2})? (AM|PM) to (\d{1,2}):?(\d{2})? (AM|PM)$", s, re.IGNORECASE)    # {1,2} means 1 or 2 characters
    if time:    # if format is wrong, wont return true.
        # ValueErrors
        if time.group(2):
        ...
        #time1   converting first time to 24H format
        ...
        #time2   converting second time to 24H format
